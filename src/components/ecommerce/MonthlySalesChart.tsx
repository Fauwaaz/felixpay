...
export default function MonthlySalesChart() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (data.success) {
          setSeries([
            { name: "Payin", data: data.data.payin },
            { name: "Payout", data: data.data.payout },
          ]);
        }
      } catch (err) {
  ...
  const options: ApexOptions = {
    colors: ["#22c55e", "#ef4444"], // Payin green, Payout red
    chart: {
    ...
    <div className="overflow-hidden  h-full rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      ...
      {loading ? (
        ...
        <div className="max-w-full overflow-x-auto custom-scrollbar">
          <div className="-ml-5 min-w-[650px] xl:min-w-full pl-4">
