...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!data.mid || !data.orderid || !data.sign)
      return NextResponse.json({ code: 1, message: "mid, orderid, sign required" });
    ...
    const [rows] = await db.query("SELECT * FROM transactions WHERE orderid = ?", [data.orderid]);
    await db.end();

    if (!rows.length) return NextResponse.json({ code: 7, message: "Order not found" });

    return NextResponse.json({ code: 0, status: "success", data: rows[0] });
